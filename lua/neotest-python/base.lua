...
local function get_script_path(filename)
  ...
  error(("%s not found"):format(filename))
end

---@return string
local get_config_loading_script_path = function()
  return get_script_path("get_pytest_options.py")
end

---@return string
function M.get_neotest_script_path()
  return get_script_path("neotest.py")
end
...
local function scan_test_function_pattern(runner, config, python_command)
  ...
  if runner == "pytest" and config.pytest_discovery then
    local cmd = vim.tbl_flatten({ python_command, get_config_loading_script_path() })
